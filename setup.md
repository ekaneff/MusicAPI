...
The final step is making sure to include the project's public SSH key in your server's `authorized_keys` file to allow CodeShip access to the server. You can find this public key under `Project Settings`, and then under `General`. Copy the public key to your clipboard. 

Now, SSH into your server and navigate to the hidden `.ssh` folder: 
...
>Note that this needs to be done for both the staging and production servers.
